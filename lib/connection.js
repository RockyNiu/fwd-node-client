...
Connection.prototype.request = function(method, args, response) {
	...
	var reqId = this.requestId([method, args]);
	var request = JSON.stringify([method, args, reqId]);
 ...
 * @return mixed
 */
Connection.prototype.requestId = function(setParams) {

	if (setParams !== undefined) {
	    var hashId = crypto.pseudoRandomBytes(20).toString();
	    this.lastRequestId = crypto.createHash('md5')
			.update(JSON.stringify(hashId, setParams))
			.digest('hex');
	}
